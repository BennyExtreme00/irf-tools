...
from pyqtconsole.console import PythonConsole, Thread
...
class MainWindow(QMainWindow):
    ...
    def auto_runes(self):
        confirmbox = pymsgbox.alert(text='Coming Soon', button='OK')
        Notification(
            title="IRF Tools",
    ...
    def insta_quit(self):
        Notification(
            title="IRF Tools",
            description = "Insta Quit enabled (Key: ALT+F4)",
            duration = 3,
        ).send()
        while self.ui.instaQuitManager.currentIndex() == 1:
    ...
    def instalocker_blind(self):
        ...
        if self.ui.laneSelector.currentIndex() == 0:
            self.lane = "Top"
        elif self.ui.laneSelector.currentIndex() == 1:
            self.lane = "Jungle"
        ...
        if data.status_code == 200:
            Notification(
                title="IRF Tools",
                description = "Instalocking {}, and spamming {}.".format(championName, self.lane),
                duration = 3,
            ).send()
